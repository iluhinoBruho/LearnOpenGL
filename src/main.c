...
int main()
{
    ...
    */
    ...
    for(int i = 0; i < NUM_THREADS; ++i){
        ...
        args[i].cond = &cond;
        args[i].mutex = &mutex;
        args[i].numthreads = NUM_THREADS; //const
        args[i].counter = &counter;
        args[i].exit_code = &exit_code;
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Wait for all threads to finish iter-work
        pthread_mutex_lock(&mutex);
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        //TODO BREAK FOR THREADS TRUELOOP
